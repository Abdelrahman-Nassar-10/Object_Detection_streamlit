    ...
    def detect_objects_tf(self, image_pil: Image.Image, confidence: float = 0.5) -> Tuple[List, List, List, dict]:
        ...
        try:
            ...
            if isinstance(results, dict):
                boxes = results['detection_boxes'][0].numpy()      # [N,4] ymin,xmin,ymax,xmax (0–1)
                classes = results['detection_classes'][0].numpy()  # [N]
                scores  = results['detection_scores'][0].numpy()   # [N]
            else:
                # typical tuple: (boxes, scores, classes, num)
            ...
            for i in range(len(scores)):
                ...
                elif 1 <= class_id-1 < len(COCO_CLASSES):
